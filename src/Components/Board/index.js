...
class Board extends Component {
  ...
  createBoard = props => {  //create a grid for the layout of our board based off the number of colums and rows passed in from props
    let board = [];         
    ...
    //console.log(board);
    //after the board is created, time to add mines randomly!
  ...
  open = cell => {
    ...
    asyncCountMines.then(numberOfMines => {
      ...
      if (current.hasMine && this.props.openCells === 0) {
        console.log("cell already has a mine! restart!!!");
  ...
  openAroundCell = cell => {
    ...
    // now we are going to go through each and open cells one by one until we find one with a mine, then we stop
    for(let row = -1; row <= 1; row++) {
      for(let col = -1; col <= 1; col++) {
        if(cell.y + row >= 0 && cell.x + col >= 0) {
          if(
